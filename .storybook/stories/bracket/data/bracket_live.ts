import type { NxGraphInput } from '@graph-render/types';

const defaultSize = { width: 280, height: 100 };
const finalSize = { width: 280, height: 100 };

// Tournament with live matches and tiebreaks
export const bracketGraphLive: NxGraphInput = {
  nodes: {
    'qf-1': {
      size: defaultSize,
      meta: {
        stage: 'QF',
        players: [
          { name: 'Nour El Sherbini', seed: 1, country: 'EGY' },
          { name: 'Hania El Hammamy', seed: 8, country: 'EGY' },
        ],
        sets: [
          [11, 7],
          [11, 9],
        ],
        status: 'completed',
      },
    },
    'qf-2': {
      size: defaultSize,
      meta: {
        stage: 'QF',
        players: [
          { name: 'Nouran Gohar', seed: 4, country: 'EGY' },
          { name: 'Amanda Sobhy', seed: 5, country: 'USA' },
        ],
        sets: [
          [11, 9],
          [9, 11],
          [11, 7],
        ],
        status: 'completed',
      },
    },
    'qf-3': {
      size: defaultSize,
      meta: {
        stage: 'QF',
        players: [
          { name: 'Nour El Tayeb', seed: 2, country: 'EGY' },
          { name: 'Camille Serme', seed: 7, country: 'FRA' },
        ],
        sets: [
          [12, 10],
          [11, 8],
        ],
        tiebreaks: [[3, 1], null], // First set had a tiebreak
        status: 'completed',
      },
    },
    'qf-4': {
      size: defaultSize,
      meta: {
        stage: 'QF',
        players: [
          { name: 'Raneem El Welily', seed: 3, country: 'EGY' },
          { name: 'Joelle King', seed: 6, country: 'NZL' },
        ],
        sets: [
          [11, 9],
          [8, 11],
          [11, 7],
        ],
        status: 'completed',
      },
    },
    'sf-1': {
      size: defaultSize,
      meta: {
        stage: 'SF',
        players: [
          { name: 'Nour El Sherbini', seed: 1, country: 'EGY' },
          { name: 'Nouran Gohar', seed: 4, country: 'EGY' },
        ],
        sets: [
          [11, 8],
          [9, 11],
          [8, 5],
        ],
        status: 'live',
        currentSet: 2, // Third set is being played
      },
    },
    'sf-2': {
      size: defaultSize,
      meta: {
        stage: 'SF',
        players: [
          { name: 'Nour El Tayeb', seed: 2, country: 'EGY' },
          { name: 'Raneem El Welily', seed: 3, country: 'EGY' },
        ],
        sets: [
          [13, 11],
          [9, 11],
          [11, 7],
        ],
        tiebreaks: [[2, 0], null, null], // First set had a tiebreak
        status: 'completed',
      },
    },
    final: {
      size: finalSize,
      meta: {
        stage: 'Final',
        players: [
          { name: 'TBD', seed: 0, country: 'TBD' },
          { name: 'TBD', seed: 0, country: 'TBD' },
        ],
        sets: [],
        status: 'upcoming',
      },
    },
  },
  adj: {
    'qf-1': { 'sf-1': { id: 'qf-1-sf-1', type: 'undirected', sourcePlayer: 0 } },
    'qf-2': { 'sf-1': { id: 'qf-2-sf-1', type: 'undirected', sourcePlayer: 0 } },
    'qf-3': { 'sf-2': { id: 'qf-3-sf-2', type: 'undirected', sourcePlayer: 0 } },
    'qf-4': { 'sf-2': { id: 'qf-4-sf-2', type: 'undirected', sourcePlayer: 0 } },
    'sf-1': { final: { id: 'sf-1-final', type: 'undirected', sourcePlayer: 0 } },
    'sf-2': { final: { id: 'sf-2-final', type: 'undirected', sourcePlayer: 0 } },
  },
};
