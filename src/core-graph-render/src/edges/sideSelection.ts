import { PositionedNode, Point, Size, NodeSide, EdgeRoutingContext } from '@graph-render/types';
 ...
 */
export function sortSidesByDistance(
  node: PositionedNode,
  size: Size,
  targetPoint: Point
): NodeSide[] {
...
/**
 * Apply directional preference for source sides (prefer left for directed edges)
 */
export function applyDirectionalPreference(sides: NodeSide[], isDirected: boolean): NodeSide[] {
  if (!isDirected) return sides;
  return [...sides].sort((a, b) => {
    const weight = (side: NodeSide) => (side === NodeSide.Left ? 1 : 0);
 ...
 */
export function findNonIntersectingSides(
