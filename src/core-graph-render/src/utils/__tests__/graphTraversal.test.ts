...
import { groupEdgesByTarget, sortEdgesBySourcePosition } from '../graphTraversal';

const makeEdge = (id: string, source: string, target: string): PositionedEdge => ({
  id,
  source,
  target,
  points: [],
...
describe('groupEdgesByTarget', () => {
  it('groups edges by their target id', () => {
    const edges = [makeEdge('e1', 'a', 'c'), makeEdge('e2', 'b', 'c'), makeEdge('e3', 'a', 'd')];
