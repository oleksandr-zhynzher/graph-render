...
export const TournamentBracket = React.memo<TournamentBracketProps>(function TournamentBracket({
  ...
  }, [enrichedGraph, exportVertexComponent, isDarkMode, mergedConfig, nodeRenderMode, vertexComponent]);
  ...
  return (
    <BracketThemeProvider mode={isDarkMode ? 'dark' : 'light'}>
      ...
      >
        ...
        />
        <div ref={wrapperRef}>
          <Graph graph={enrichedGraph} vertexComponent={resolvedVertexComponent} config={mergedConfig} />
        </div>
      </div>
    </BracketThemeProvider>
  );
