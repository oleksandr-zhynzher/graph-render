...
import { normalizeMatchMeta } from '../utils/squash/normalizeMatchMeta';
...
describe('normalizeMatchMeta', () => {
  ...
  it('normalizes a valid sets array', () => {
    const meta = normalizeMatchMeta({ sets: [[6, 4], [3, 6]] });
    expect(meta.sets).toHaveLength(2);
    expect(meta.sets[0]).toEqual([6, 4]);
  });

  it('throws for malformed set entry', () => {
    expect(() => normalizeMatchMeta({ sets: [[6]] })).toThrow(/sets\[0\] must contain exactly two scores/);
  });
  ...
  it('throws for non-numeric currentSet', () => {
    expect(() => normalizeMatchMeta({ currentSet: 'first' })).toThrow(/currentSet must be a finite number/);
  });
